...
pub async fn truncate_tables(pool: &PgPool) -> Result<()> {
    sqlx::query("TRUNCATE TABLE events, view_snapshots, handler_stream_offsets")
        .execute(pool)
        .await?;
...
pub async fn create_test_database(
    ...
    test_db_name: &str,
) -> Result<PostgresEventStore> {
    // Connect to postgres to create the database
    let options = PgConnectOptions::from_str(admin_connection_string)?.disable_statement_logging();
...
pub async fn create_test_database_with_schema(
    admin_connection_string: &str,
    test_db_name: &str,
    schema_path: &str,
) -> Result<PostgresEventStore> {
...
fn extract_db_name_from_connection_string(connection_string: &str) -> Result<String> {
    let url = Url::parse(connection_string).context("Failed to parse connection string as URL")?;
...
pub fn initialize_logger() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            "info,ask_cqrs::view_event_handler=debug,ask_cqrs::store::postgres_view_store=debug",
        )
        .try_init();
}
